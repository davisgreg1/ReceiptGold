...
interface SubscriptionDocument {
  userId: string;
  currentTier: 'starter' | 'growth' | 'professional' | 'teammate';
...
export const onUserDelete = functionsV1.auth.user().onDelete(async (user: admin.auth.UserRecord) => {
  try {
    ...
    Logger.error('Error permanently deleting user data:', { error: (error as Error) });
  }
});
...
async function handleRevenueCatSubscriptionUpdated(event: any): Promise<void> {
  try {
    ...
    const currentTier = currentSub.data()?.currentTier || 'starter';
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (shouldUpdateUsageLimits(currentTier, tier)) {
        ...
        transaction.set(usageRef, {
          limits: subscriptionTiers[tier]?.limits || subscriptionTiers.starter.limits,
          updatedAt: admin.firestore.FieldValue.serverTimestamp()
        }, { merge: true });
  ...
  try {
    ...
    const updateData: any = {
      ...
      billing: {
        ...
        latestPurchaseDate: new Date(event_timestamp_ms),
        expiresDate: expiresDate, // Use actual expiration date from RevenueCat renewal
        currentPeriodEnd: expiresDate, // Set current period end to match expiration
        isActive: true,
        willRenew: true,
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (doc.exists) {
        ...
        const createData = {
          ...
          currentTier: tier,
          limits: subscriptionTiers[tier]?.limits || subscriptionTiers.starter.limits,
          features: subscriptionTiers[tier]?.features || subscriptionTiers.starter.features,
          history: [{
            tier: tier,
            startDate: new Date(event_timestamp_ms),
            endDate: null,
...
async function handleRevenueCatTransferDirect(event: any): Promise<void> {
  try {
    ...
    // Extract Firebase user IDs (skip anonymous RevenueCat IDs)
    const originalUserId = transferred_from.find((id: string) => id && !id.startsWith('$RCAnonymousID:'));
    const newUserId = transferred_to.find((id: string) => id && !id.startsWith('$RCAnonymousID:'));
...
export const cleanupOrphanedUserData = onCall<{
  ...
  try {
    ...
    if (userExists && !confirmDelete) {
      throw new HttpsError('failed-precondition', 'User still exists in Firebase Auth. Use confirmDelete=true if you really want to delete their data.');
    }
